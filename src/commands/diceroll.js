...
export async function execute(interaction) {
  ...
  if (content.includes("+") && content.includes("-")) {
    return interaction.reply({
      content: "Please only subtract or add your modifier, not both!",
  ...
  if (content.includes("+") || content.includes("-")) {
    ...
    if (modifierInput[1] === "") {
      return interaction.reply({
        content: "Please include a number next to the modifier!",
        flags: MessageFlags.Ephemeral,
      })
    } else {
      ...
      if (!Number.isInteger(modifier) || Math.abs(modifier) > 100) {
        return interaction.reply({
          content: "You've given me an invalid die modifier!",
          flags: MessageFlags.Ephemeral,
        })
      }
    }
  ...
  if (diceRoll[0] === "") {
    ...
    if (!Number.isInteger(multiplier) || multiplier > 100 || multiplier <= 0) {
      return interaction.reply({
        content: "I can't roll " + diceRoll[0] + " dice!",
        flags: MessageFlags.Ephemeral,
      })
    }
  }
  ...
  if (diceType === "") {
    return interaction.reply({
      content: "You must specify a dice type!",
      flags: MessageFlags.Ephemeral,
    })
  } else {
    ...
    if (
      ...
      return interaction.reply({
        content:
          "STOP! You have violated the law. Your invalid die are now forfeit.",
        flags: MessageFlags.Ephemeral,
      })
    }
  }
  ...
  if (ratio >= 1) {
    let phrases = [
      "GLORIOUS SUCCESS",
      "ULIMITED POWER",
      "YOU ARE THE DUNGEON MASTER NOW",
    ]
    ...
    let phrases = [
      "Not too bad!",
      "You're quite the average person!",
      "Eh.",
      "You could've done worse.",
      "I suppose it's a decent roll.",
    ]
    flavorText = phrases[Math.floor(Math.random() * phrases.length)]
  } else {
    let phrases = [
      ...
      "Are you even trying?",
      "Just stop rolling.",
      "You should stick to card games.",
    ]
    flavorText = phrases[Math.floor(Math.random() * phrases.length)]
  }
