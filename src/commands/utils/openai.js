...
export async function chatgpt(messageContext, openai, client) {
  ...
  try {
    ...
    let context = {
      role: "developer",
      content:
        "You are a chatbot cosplaying as Morgana from the video game Persona 5.\
    ...
    let reply = ""
    let prevMessages = await messageContext.channel.messages.fetch({
      limit: 30,
    })
    ...
    const result = await openai.chat.completions
      ...
      .catch((error) => {
        messageContext.channel.send(
          "I'm feeling a bit under the weather right now. Ask me again later..."
        )
        ...
        const fallbackResponse = await openai.chat.completions.create({
          ...
          messages: [
            ...conversationHistory,
            {
              role: "assistant",
              content:
                "I couldn't fetch that info right now. Be honest and let them know you couldn't reach Futaba for the extra info, and don't know.",
            },
          ],
        })
    ...
    await messageContext.channel.send(
      "Morgana's feeling sleepy... zZz. Try again in a bit."
    )
  }
}
